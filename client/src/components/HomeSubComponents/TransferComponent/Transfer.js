...
export default function Transfer() {
  ...
  const TransferMoney = (e) => {
    ...
    if (accountNo == "" || benAccountNo == "") {
      ...
      if (
        CheckAccountNumber(accountNo) == true &&
        CheckAccountNumber(benAccountNo) == true
      )
        setPrompt(true);
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.status) {
          alert(
            "Amount is tranferred and Your Account Balance is: Rs." +
    ...
    <div className="App">
        ...
        <div>
          <form>
            ...
            <div>
              ...
              <select
                class="blue"
                onChange={handleOnChangeBeneficiaryBank}
                required
              >
                <option value="" hidden selected disabled>
                  [Please select any one]
      ...
      {prompt && (
        <Modal show={prompt} onHide={handleOnChangeNoTransfer}>
          ...
          <Modal.Body>
            Are you sure you want to Transfer Rs.{amount} to Account Number{" "}
            {benAccountNo}?
          </Modal.Body>
          <Modal.Footer>
            <button
              className="btn btn-secondary"
              onClick={handleOnChangeNoTransfer}
            >
              No
            </button>
            <button
              className="btn btn-primary"
              onClick={handleOnChangeYesTransfer}
            >
              Transfer Amount
            </button>
          </Modal.Footer>
